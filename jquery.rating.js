	...
	$.fn.rating = function(instanceSettings){
		...
		this.each(function(i){
			...
			/*
				...
				the problem is, rails form fields look like this:
				
				mymodel[myfield]
				
				so when the drain and fill events go to create the custom css class name, you end up with things like:
				
				star_group_mymodel[myfield]
				
				for the class name.  brackets [] are illegal in css class names, so this makes the plugin fail.  the code below changes the above mangled class name into this:
				
				star_group_mymodel_myfield
				
				and it works like a charm.  the real_n variable is created so that the hidden form field will still work properly.
			...
			*/
			...
			if(i == 0){
				...
				$.rating.groups[n].valueElem = $('<input type="hidden" name="' + real_n + '" value=""' + (settings.readOnly ? ' disabled="disabled"' : '') + '>');
